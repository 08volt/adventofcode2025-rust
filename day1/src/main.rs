...
impl Dial {
    ...
    fn count_pass_0_with_steps( &self, steps: u16, is_right: bool ) -> u16 {
        ...
        if remaining_steps == 0 || self.position == 0  || (is_right && self.position + remaining_steps < 100) || (!is_right && self.position > remaining_steps) {
            return full_cycles;
        }
...
fn main() {
    ...
    let steps : Vec<(u16, bool)> = lines.iter().map( | line | {
